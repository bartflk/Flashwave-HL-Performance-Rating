...
| Q16b | **Aim for everyone, not just you** | large | Needs a `shooter` column on `demo_aim` and the aim pass run for every player in an STV rather than the owner alone. Then the player dropdown means what it implies on every tab. |
