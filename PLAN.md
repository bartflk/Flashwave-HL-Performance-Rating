...
| Q14 | **Look other players up** (Flashy) | large | A light trends.tf inside the app: search a player, see an overview, browse their logs. Most of what it needs is already here — every stored log holds 17 other players, and Q9 just built the "how does this player rate over their other games" query that a profile page would be made of. What is *not* here is anything of theirs the owner did not play in, so the honest first version is "this player, in your matches": their classes, their rating per class against your pool, their best games, who they play with, and the logs you both appeared in. Fetching their full history from logs.tf is a second step and a much larger one — it is somebody else's 700 logs per player and logs.tf's patience is finite. **To settle:** whether a looked-up player is rated against your pool (cheap, honest, and says so) or against their own (needs their whole history). Starting with the former. |
