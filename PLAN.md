...
| Q15 | **An error console, and telling people when something fails** (Flashy) | medium | Three separate gaps. **1. Nothing catches a crash** — there is no error boundary anywhere, so one thrown exception unmounts the whole window and leaves a black rectangle (KamikaZe's screenshot). A boundary round the page and one per `Fold` turns that into "this panel failed". **2. Nothing is recorded** — a failed log fetch, a failed demo read, a failed part are logged to `tracing` inside the backend where no user will ever see them. Keep a ring buffer of the last ~200 warnings and errors, surface a count in the header, and list them in Settings with what was being done, the log id, and the message. **3. Nothing is shareable** — a "Copy report" button that puts the recent errors, the app version, the model version and the database counts on the clipboard as markdown, so a Discord report is one paste instead of five screenshots. The sync already reports failures per log (`Progress::FetchFailed`); this is the same idea for everything else. |
| Q16 | **The Aim tab ignores the selected player** (zaag) | medium | Confirmed: `AnalysisPanel` renders `<Aim a={a} logId={a.logId} slice={slice} />` and never passes `player`, so changing the player leaves the aim cards showing *your* numbers under their name — "my sniper has 28° crosshair deviation". The reason it was never wired is structural: `demo_aim` has no shooter column, because the pass was written for "the aim behind each of **the owner's** kills". Two steps. **Now:** the tab must stop lying — name whose aim it is, and when someone else is selected say that aim is only read for your own kills. **Then:** an STV demo carries every player's viewangles, so a `shooter` column and a pass over all players would make the tab mean what the dropdown implies. That is the real fix and it is a demo-parsing job, close to Q3. |
...
| Q19 | **The match page has too much furniture** (Flashy) | small | Three cuts, all about clutter. **1. "Combined from N logs"** is a whole panel to say something that belongs in the header, next to the logs.tf and demos.tf links — the part log ids as links, and nothing else. **2. The "Reading" dropdown** is its own panel for one `<select>`; it belongs in the scoreboard's header, where the thing it scopes actually is. **3. Panel titles** need a pass: "What the rating is made of", "Officials, scrims and pugs", "Kill by kill" and "Damage and kills by class" are sentences where a label would do. |
| Q20 | **Fewer explanations, everywhere** (Flashy) | small | A first pass cut about twenty in 0.4.0, and the rule it used is worth keeping: cut anything that explains what the reader is already looking at, keep what they cannot infer. The ones left to do are the longer ones inside panels — the breakdown's footnote, the fights explainer, the Setup screen, and the `title=` tooltips, which are the same prose hidden behind a hover. |
